...
class ServicePlanner:
    ...
    def generate_employees(self, count: int | None = None, seed: int | None = None) -> list[Employee]:
        ...
        load_all_from_db = getattr(self.config, 'LOAD_ALL_FROM_DB', False)
        load_from_db = load_all_from_db
    ...
    def assign_vehicles(self) -> list[Vehicle]:
        ...
        load_all_from_db = getattr(self.config, 'LOAD_ALL_FROM_DB', False)
        load_vehicles_from_db = load_all_from_db
        randomize_db_vehicle_assignment = load_all_from_db

        existing_vehicles: list[Vehicle] = []
        if load_vehicles_from_db and self.use_database and self.vehicle_repo:
            try:
                existing_vehicles = self.vehicle_repo.find_all(limit=5000)
                if existing_vehicles:
                    print(f"[5] Reusing {len(existing_vehicles)} vehicles from database...")
                    if randomize_db_vehicle_assignment:
                        random.shuffle(existing_vehicles)
                        print("[5] Randomized DB vehicle assignment order")
            except Exception as e:
                print(f"[5] Could not load vehicles from database: {e}")

        print(f"[5] Assigning vehicles (capacity: {cap})...")
        self.vehicles = []

        next_new_vehicle_id = 1
        if existing_vehicles:
            next_new_vehicle_id = max(v.id for v in existing_vehicles) + 1

        for i, c in enumerate(self.clusters):
            active_count = c.get_employee_count(include_excluded=False)
            if active_count > 17:
                target_capacity = 27
                target_type = "Midibus"
            else:
            ...
            if i < len(existing_vehicles):
                base = existing_vehicles[i]
                v = Vehicle(
                    id=base.id,
                    capacity=target_capacity,
                    vehicle_type=target_type,
                )
                v.driver_name = base.driver_name
                v.plate_number = base.plate_number
            else:
                v = Vehicle(id=next_new_vehicle_id, capacity=target_capacity, vehicle_type=target_type)
                next_new_vehicle_id += 1
            v.assign_cluster(c)
    ...
    def clear_database(self):
        ...
        preserve_vehicles = getattr(self.config, 'LOAD_ALL_FROM_DB', False)
        vehicle_table_sql = "" if preserve_vehicles else "                vehicles,\n"
...
{vehicle_table_sql}
            ...
            """)
            if preserve_vehicles:
                print("    ✓ Route data cleared, employees and vehicles preserved")
            else:
